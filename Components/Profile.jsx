    ...
    const fetchShipmentsCount = async () => {
      if (getShipmentsCount) {
        try {
          ...
          setCount(allData || 0); // Default to 0 if data is not available
        } catch (error) {
          console.error("Error fetching shipments count:", error.message || error);
        }
      }
    };
          ...
          <div className="max-w-sm mx-auto py-3 space-y-3 text-center">
            <div className="flex flex-col items-center pb-10">
              <Image
                ...
                alt="Profile Image"
              />
              <h5 className="mb-1 text-xl font-medium text-gray-900">Welcome Trader</h5>
              <span className="text-sm text-gray-500">{currentUser || "Guest User"}</span>
